 ...
 * 构建的时候自动为你拼写一些功能代码，如get set 方法等。 安装方法： file -- settings -- plugins -- 搜索lombok
 * lombok常用注解还有： @Getter/@Setter : 注解在类上, 为类提供读写属性； @ToString : 注解在类上, 为类提供
 * toString() 方法 @Log4j : 注解在类上, 为类提供一个属性名为 log 的 log4j 的日志对象 @Slf4j : 注解在类上,
 * 为类提供一个属性名为 log 的 log4j 的日志对象 @NonNull : 注解在参数上, 如果该类参数为 null , 就会报出异常, throw
 * new NullPointException(参数名)
 *
